...
<?php endif ?>
        ...
        <form class="snp-subscribeform snp_subscribeform" action="<?php if(snp_get_option('ml_manager') == 'html'){echo snp_get_option('ml_html_url');}else{echo '#';}?>" method="post" <?php if(snp_get_option('ml_manager') == 'html' && snp_get_option('ml_html_blank')){echo ' target="_blank"';}?>>
...
if (!empty($POPUP_META['snp_border_size']) && $POPUP_META['snp_border_size']>0 && !empty($POPUP_META['snp_border_color'])) {
    ...
    echo '.snp-pop-'.$ID.' .snp-'. $themeid .' { border: '.$POPUP_META['snp_border_size'].'px solid '.$POPUP_META['snp_border_color'].';}'."\n";
...
{
    echo '.snp-pop-'.$ID.' .snp-'. $themeid .' .snp-newsletter-content header h2 {font-size: '.$POPUP_META['snp_header_font']['size'].'px; color: '.$POPUP_META['snp_header_font']['color'].';}'."\n";
}
if (!empty($POPUP_META['snp_maintext_font']['size']))
{
    ...
    echo '.snp-pop-'.$ID.' .snp-'. $themeid .' .snp-newsletter-content p small {color: '.$POPUP_META['snp_maintext_font']['color'].';}'."\n";
}
if (!empty($POPUP_META['snp_fields_font']['size']))
{
    ...
    echo '.snp-pop-'.$ID.' .snp-'. $themeid .' form textarea { font-size: '.$POPUP_META['snp_fields_font']['size'].'px; color: '.$POPUP_META['snp_fields_font']['color'].';}'."\n";
}
if (!empty($POPUP_META['snp_submit_button_font']['size']))
{
    echo '.snp-pop-'.$ID.' .snp-'. $themeid .' .snp-submit { font-size: '.$POPUP_META['snp_submit_button_font']['size'].'px; color: '.$POPUP_META['snp_submit_button_font']['color'].';}'."\n";
}
if (!empty($POPUP_META['snp_submit_button_color']))
{
    echo '.snp-pop-'.$ID.' .snp-'. $themeid .' .snp-submit { background-color: '.$POPUP_META['snp_submit_button_color'].';}'."\n";
}
...
{
    echo '.snp-pop-'.$ID.' .snp-'. $themeid .' { background-color: '.$POPUP_META['snp_bg_color1'].';}'."\n";
}
if (!empty($POPUP_META['snp_fields_border_color']))
{
    echo '.snp-pop-'.$ID.' .snp-'. $themeid .' form input[type="text"],'."\n";
    echo '.snp-pop-'.$ID.' .snp-'. $themeid .' form select,'."\n";
    echo '.snp-pop-'.$ID.' .snp-'. $themeid .' form textarea { border-color: '.$POPUP_META['snp_fields_border_color'].';}'."\n";
}
...
{
    echo '.snp-pop-'.$ID.' .snp-'. $themeid .' .snp-security-note small { font-size: '.$POPUP_META['snp_security_note_font']['size'].'px; color: '.$POPUP_META['snp_security_note_font']['color'].';}'."\n";
}

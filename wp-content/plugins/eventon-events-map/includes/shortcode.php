...
class evo_em_shortcode{
	...
	// generate calendar
		function generate_calendar($atts){
			global $evoemap, $eventon;			
			$evoemap->is_running_em=true;			
			
			// add additional default values to accepted shortcode variables
			add_filter('eventon_shortcode_defaults', array($this,'evoEM_add_shortcode_defaults'), 10, 1);
			...
			ob_start();				
				echo $evoemap->frontend->generate_evo_em($args);			
			return ob_get_clean();
					
		}
	// add new default shortcode arguments
		function evoEM_add_shortcode_defaults($arr){

...
<div class="wrap woocommerce">
	...
	<h2 class="nav-tab-wrapper woo-nav-tab-wrapper">
		...
		<a href="admin.php?page=<?php echo wc_cart_notices()->id; ?>&amp;tab=new" class="nav-tab <?php echo ( 'new' === $tab ) ? 'nav-tab-active' : ''; ?>">
			<?php esc_html_e( 'New Notice', 'woocommerce-cart-notices' ); ?>
		</a>
	</h2>
	...
	<?php if ( 'list' === $tab ) : ?>
		...
		<h3><?php esc_html_e( 'Cart Notices', 'woocommerce-cart-notices' ); ?></h3>
							...
							<td>
								...
								switch ( $notice->type ) :
									...
									case 'minimum_amount':
										...
										echo sprintf( esc_html__( 'Minimum order amount: %s', 'woocommerce-cart-notices' ), wc_cart_notices()->get_minimum_order_amount( $notice ) ? get_woocommerce_currency_symbol() . wc_cart_notices()->get_minimum_order_amount( $notice ) : 'none configured' ) . '<br/>';
										/* translators: %s - formatted amount quantity */
	...
	<?php elseif ( 'new' === $tab || 'edit' === $tab ) : ?>
		...
		<form action="admin-post.php" method="post">
			...
			<h3><?php echo 'new' === $tab ? esc_html__( 'Create a New Cart Notice', 'woocommerce-cart-notices' ) : esc_html__( 'Update Cart Notice', 'woocommerce-cart-notices' ); ?></h3>
				...
				<tbody>
					...
					<tr valign="top">
						...
						<td>
							...
							<p class="description minimum_amount_notice_data notice_data" style="<?php echo 'minimum_amount' !== $notice->type ? 'display:none;' : ''; ?>">
								<?php esc_html_e( 'This notice will appear on the cart/checkout pages only when the order total is less than the Minimum Order Amount, and/or is greater than or equal to the Threshold Amount and is convenient for encouraging customers to increase their order to qualify for free shipping.', 'woocommerce-cart-notices' ); ?>
							</p>
								...
								<ul>
									<?php
										...
										foreach( $shortcodes as $shortcode => $help_tip ) {
											...
											echo '<li><strong>' . $shortcode . '</strong> ' . SV_WC_Plugin_Compatibility::wc_help_tip( $help_tip ) . '</li>';
										}
					...
					<tr valign="top" class="minimum_amount_notice_data notice_data" style="<?php echo 'minimum_amount' !== $notice->type ? 'display:none;' : ''; ?>">
						<th scope="row">
							<label for="minimum_order_amount">
								<?php esc_html_e( 'Minimum Order Amount', 'woocommerce-cart-notices' ); ?>
							</label>
						</th>
						<td>
					...
					<tr valign="top" class="minimum_amount_notice_data notice_data" style="<?php echo 'minimum_amount' !== $notice->type ? 'display:none;' : ''; ?>">
						...
						<td>
							...
							<span class="description">
								<?php _e( 'Optional threshold amount to activate the notice.  If set, the cart must contain <em>at least</em> this total amount for the notice to be displayed.', 'woocommerce-cart-notices' ); ?>
							</span>
						</td>
					</tr>
					...
					<tr valign="top" class="products_notice_data notice_data" style="<?php echo 'products' !== $notice->type ? 'display:none;' : ''; ?>">
						<th scope="row">
							<label for="product_ids">
								<?php esc_html_e( 'Products', 'woocommerce-cart-notices' ); ?>
							</label>
						</th>
						<td>
							<input type="hidden"
					...
					<tr valign="top" class="products_notice_data notice_data" style="<?php echo 'products' !== $notice->type ? 'display:none;' : ''; ?>">
						...
						<td>
							<select id="shipping_countries" name="shipping_countries[]" class="wc-enhanced-select" multiple="multiple" data-placeholder="<?php esc_attr_e( 'Choose Countries&hellip;', 'woocommerce-cart-notices' ) ?>">
								<?php
					...
					<tr valign="top" class="categories_notice_data notice_data" style="<?php echo 'categories' !== $notice->type ? 'display:none;' : ''; ?>">
						<th scope="row">
							<label for="category_ids">
								<?php esc_html_e( 'Categories', 'woocommerce-cart-notices' ); ?>
							</label>
						</th>
						<td>
							<input type="hidden"
							       class="sv-wc-enhanced-search"
							       name="category_ids" data-multiple="true"
				...
				<p style="float:left;padding-top:8px;margin-right:8px;margin-top:0;">
					...
					if ( 'minimum_amount' === $notice->type ) {
						...
						if ( is_numeric( $minimum_order_amount ) && ! is_numeric( $threshold_order_amount ) ) {
							...
							printf(
								/* translators: Placeholders: %1$s Threshold order amount, %2$s Minimum order amount */

...
class WC_Reviews_List_Table extends WP_List_Table {
	...
	public function restrict_manage_reviews() {
		...
		?>
		<input type="hidden" class="wc-product-search" name="p" data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'woocommerce-product-reviews-pro' ); ?>" data-selected="<?php echo esc_attr( $current_product_name ); ?>" value="<?php echo esc_attr( $current_product ); ?>" data-action="woocommerce_json_search_products" data-allow_clear="true" >
		<?php
	}
	...
	/**
	 * Render the comment column HTML
	 *
	 * @param WP_Comment $comment
	 */
	public function column_comment( $comment ) {
		...
		<div class="submitted-on">
			<?php
			/* translators: Placeholders: %1$s opening <a> link tag, %2$s comment date, %3$s comment time, %4$s closing <a> link tag */
		...
		<?php if ( $rating && 'yes' === get_option( 'woocommerce_enable_review_rating' ) ) :
			...
			// if this is a review with a rating, display the star rating:
			?>
			<span class="star-rating" title="<?php
				/* translators: Placeholder %d - rating (e.g. 'Rated 4 out of 5') */
				...
				<span style="width:<?php echo esc_attr( ( $rating / 5 ) * 100 ); ?>%"><?php
					/* translators: Placeholder %d - rating (e.g. '4 out of 5') */
					printf( __( '%d out of 5', 'woocommerce-product-reviews-pro' ), $rating ) ; ?></span>
			</span>

		<?php endif;

		do_action( 'woocommerce_reviews_list_before_comment_text' );

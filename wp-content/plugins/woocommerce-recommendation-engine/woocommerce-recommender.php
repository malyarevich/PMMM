	...
	if ( !class_exists( 'WC_Recommendation_Engine' ) ) {
		...
		class WC_Recommendation_Engine {
			...
			public function load_plugin_textdomain() {
				load_plugin_textdomain( 'wc_recommender', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
			}
			...
			public function on_woocommerce_init() {
				if ( (!is_admin() || defined( 'DOING_AJAX' ) ) && !defined( 'DOING_CRON' ) ) {
					...
					if ( !WC_Recommender_Compatibility::WC()->session->has_session() ) {
						WC_Recommender_Compatibility::WC()->session->set_customer_session_cookie( true );
					}
				}
			}
				...
				foreach ( $order_items as $item ) {
					...
					if ( $product && is_object( $product ) ) {
						if ( $product->is_type( 'variable' ) ) {
							woocommerce_recommender_record_product_ordered( $order_id, $product->id, $order->status );
							woocommerce_recommender_record_product_ordered( $order_id, $product->variation_id, $order->status );
						} else {
							woocommerce_recommender_record_product_ordered( $order_id, $product->id, $order->status );
						}
					}
			...
			public function record_update_order( $order_id ) {
				...
				foreach ( $order_items as $item ) {
					...
					if ( $product && is_object( $product ) ) {
						if ( $product->is_type( 'variable' ) ) {
							woocommerce_recommender_update_recorded_product( $order_id, $product->id, $order->status );

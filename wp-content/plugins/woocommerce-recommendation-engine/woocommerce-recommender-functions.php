...
function woocommerce_recommender_get_simularity( $current_product_id, $activity_types = array('completed', 'in-cart', 'viewed') ) {
	global $wpdb, $woocommerce_recommender;
...
class WC_Recommender_Recorder {
	...
	public function __construct() {
		...
		$this->product_ids = $wpdb->get_col( "SELECT ID FROM $wpdb->posts WHERE post_type = 'product' and post_status='publish'" );
	}
	...
	public function build_all_async() {
		...
		$endpoints = array();
		$endpoint_template = add_query_arg( array('nocache' => '%s', 'woocommerce_recommender_build_slice' => 'true', 'start' => '%d'), trailingslashit( get_site_url() ) );
		...
		foreach ( $endpoints as $url ) {
			echo 'Adding End Point: ' . $url . PHP_EOL;
	...
	function woocommerce_recommender_begin_build_simularity( $start = false, $count = 10, $skip_delete = false ) {
		global $wpdb, $woocommerce_recommender;
		...
		foreach ( $products_to_process as $product_id ) {
			echo 'Building scores for Product ID:' . $product_id;
			echo PHP_EOL;
	...
	function woocommerce_recommender_build_simularity( $current_product_id, $activity_types = array('completed', 'in-cart', 'viewed') ) {
		global $wpdb, $woocommerce_recommender;
	...
	function woocommerce_build_purchased_together( $current_product_id, $activity_types = array('completed') ) {
		...
		$key = 'wc_recommender_fpt_' . implode( '_', $activity_types ) . '_' . $current_product_id;
		$sql = $wpdb->prepare( "SELECT DISTINCT order_id FROM $woocommerce_recommender->db_tbl_session_activity WHERE order_id > 0 AND product_id = %d AND activity_type IN ('" . implode( "','", $activity_types ) . "')", $current_product_id );
		$item1 = $wpdb->get_col( $sql );
		...
		foreach ( $product_ids as $product_id ) {
			if ( $product_id != $current_product_id ) {
				...
				$item2 = $wpdb->get_col( $wpdb->prepare( "SELECT order_id FROM $woocommerce_recommender->db_tbl_session_activity WHERE order_id > 0 AND product_id = %d AND activity_type IN ('" . implode( "','", $activity_types ) . "')", $product_id ) );
...
function woocommerce_recommender_manually_build_scores() {
	...
	if ( isset( $_REQUEST['woocommerce_recommender_build'] ) && !empty( $_REQUEST['woocommerce_recommender_build'] ) ) {
		...
		if ( $next_start < $total ) {
			...
			wp_redirect( esc_url_raw( add_query_arg( array('woocommerce_recommender_build' => false, 'built' => $total) ) ) );
			die();
		}
	}
}

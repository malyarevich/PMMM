...
class WC_GZDP_Assets {
	...
	public function __construct() {
		...
		add_action( 'admin_enqueue_scripts', array( $this, 'add_admin_assets' ), 0 );
	...
	public function add_admin_assets() {
		...
		wp_register_style( 'wc-gzdp-admin', WC_germanized_pro()->plugin_url() . '/assets/css/admin/wc-gzdp-admin' . $this->suffix . '.css', array(), WC_GERMANIZED_PRO_VERSION );
		wp_enqueue_style( 'wc-gzdp-admin' );

		wp_register_style( 'wc-gzd-fontawesome', WC_germanized()->plugin_url() . '/assets/css/font-awesome.min.css', array(), WC_GERMANIZED_VERSION );

		wp_register_script( 'wc-gzdp-admin-invoice', WC_germanized_pro()->plugin_url() . '/assets/js/admin/invoice' . $this->suffix . '.js', array( 'jquery' ), WC_GERMANIZED_PRO_VERSION, true );
		...
		if ( in_array( $screen->id, array( 'shop_order', 'edit-shop_order' ) ) ) {
			...
			// Invoice JS
			wp_enqueue_script( 'wc-gzdp-admin-invoice' );
			// Order JS
			wp_enqueue_script( 'wc-gzdp-admin-order' );			

		} else if ( $screen->id == 'woocommerce_page_wc-settings' ) {

			wp_enqueue_media();
			wp_enqueue_style( 'wc-gzd-fontawesome' );

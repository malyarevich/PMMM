                    ...
                    switch ($browser_working) {
                        ...
                        case 'ie':
                            ...
                            if (strstr($browser_user_agent, 'rv:')) {
                                ...
                                $b_gecko_ua = true;
                                $this->get_set_count('set', 0);
                                $browser_number = $this->get_item_version($browser_user_agent, 'rv:', '', '');
                            } else {
                            ...
                            if ($layout_engine_nu_full) {
                                ...
                                for ($j = 0; $j < $j_count; $j++) {
                                    if (strstr($browser_user_agent, $a_trident_types[$j])) {
                                        ...
                                        $trident_type_number = $this->get_item_version($browser_user_agent, $trident_type);
                                        break;
                                    }
                                }
                        ...
                        case 'webkit':
                            ...
                            for ($j = 0; $j < $j_count; $j++) {
                                if (strstr($browser_user_agent, $a_webkit_types[$j])) {
                                    ...
                                    if ($a_webkit_types[$j] == 'chrome' && $this->get_item_math_number($webkit_type_number) >= 28) {
                                        if (strstr($browser_user_agent, 'blink')) {
                                            $layout_engine_nu_full = $this->get_item_version($browser_user_agent, 'blink');
                                            $layout_engine_nu = $this->get_item_math_number($layout_engine_nu_full);
                                        }
...
// gets which os from the browser string
    ...
    function get_html_level($pv_render_engine, $pv_render_engine_nu) {
        ...
        if (array_key_exists($pv_render_engine, $a_html5_forms) && $a_html5_forms[$pv_render_engine] <= $engine_nu
        ) {
            $html_return = 3;
        } elseif (array_key_exists($pv_render_engine, $a_html5_basic) && $a_html5_basic[$pv_render_engine] <= $engine_nu
        ) {
            $html_return = 2;
        }
        return $html_return;

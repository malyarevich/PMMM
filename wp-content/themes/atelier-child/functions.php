<?php
define('SF_LOCAL_PATH_CHILD', get_stylesheet_directory_uri());

/**
 * Loads the child theme textdomain.
 */
function my_child_theme_setup() {
	load_child_theme_textdomain( 'atelier', get_stylesheet_directory() . '/languages' );
}
    ...
    function wc_minimum_order_amount()
    {
        ...
        if (WC()->cart->total < $minimum) {
            ...
            if (is_cart()) {
                ...
                wc_print_notice(
                    sprintf('Der Mindestbestellwert (exkl. Versandkosten) beträgt 75 €.',
                        wc_price($minimum),
...
add_filter( 'woocommerce_get_price_html', 'return_custom_price_html', 100, 2 );

function return_custom_price_html( $price, $product ){
    ...
    if ( ! is_cart() && ! is_checkout() ) {
        ...
        if ( 1 < $min_value )
        {
            ...
            $new_price = $min_value * $product->price;
            ...
            if ( is_product() || is_product_category() )
            {
                ...
                if ( $product->regular_price != $product->price )
                {
                    ...
                    $old_price = '<del class="del-old-price">' . wc_price( $min_value * $product->regular_price ) . '</del> ';

                }

            }

            return  $old_price . wc_price( $new_price ) . '<span class="price-von"> ' . sprintf( __( 'pro %s Flaschen', 'swiftframework' ), $min_value ) . '</span>';
        }
    }
    return $price;
...
function woocommerce_gzd_template_single_price_unit() {
    ...
    if ( in_array( $product->product_type, array( 'simple', 'external' ) ) )
        wc_get_template( 'price-unit.php' );
}

function woocommerce_template_loop_price() {
    wc_get_template( 'price.php' );
}
...
function sf_product_price_rating() {
    ...
    <div class="product-price-wrap clearfix">
        <div itemprop="offers" itemscope itemtype="http://schema.org/Offer">
            ...
            <h3 <?php echo ( is_product() ) ? 'style="width: 100% !important; text-align:left !important;"' : ''; ?> class="price"><?php echo $product->get_price_html(); ?></h3>
...
add_filter("gform_field_validation_9_10", "validate_word_count", 10, 4);

function validate_word_count($result, $value, $form, $field){
    if (str_word_count($value) < 10) //Minimum number of words
    {
        $result["is_valid"] = false;
        $result["message"] = "Um Ihre Rezension übermitteln zu können, geben Sie mindestens 20 Wörter ein.";
    }
    return $result;
}

// Change order of product tabs
...
function my_template( $template ) {
    if( is_page('wine-course') ){
        if ( $new_template = locate_template( array( 'login-register.php' ) ) )
            return $new_template ;
    }
    if( is_page('wine-course-step') ){
        if ( $new_template = locate_template( array( 'course-step.php' ) ) )
            return $new_template ;
    }
    return $template;
}

add_action( 'gform_user_registered', 'reg_login_redirect', 10, 4 );
function reg_login_redirect(){
    wp_redirect(site_url().'/wine-course-step/');
    exit;
}

add_filter( 'gform_validation', 'is_reg_email' );
function is_reg_email( $validation_result ) {
    $form = $validation_result["form"];
    $entry = GFFormsModel::get_current_lead();
    if( $form['id'] == '11'){
        $_SESSION['user_course_mail']=$entry[7];
        $_SESSION['course_step']=1;
//        reg_login_redirect();
    }

    if( $form['id'] == '12'){
        $temp=false;
        $user  = get_user_by('email', $entry[1]);
        if($user)
            $temp=true;
        if(!$temp){
            ...
            foreach ( $form["fields"] as &$field ) {
                if ( $field["id"] == "1" ) {
                    ...
                    $field["validation_message"] = "The email does not exist";
                    break;
                }
            }
        }
        else{
            $_SESSION['user_course_mail']=$entry[1];
            if(get_user_meta($user->id, "course_step", true))
                $_SESSION['course_step'] = get_user_meta($user->id, "course_step", true);
            else
                $_SESSION['course_step'] = 1;
            $_SESSION['course_percent'] = get_user_meta($user->id, "course_percent", true);
            $_SESSION['course_score'] = get_user_meta($user->id, "course_score", true);
            reg_login_redirect();
        }
    }
...
function change_message($message,$form ) {
    if( $form['id'] >= '13' && $form['id'] <= '18')
        $message = "<div class='validation_error'>PLEASE ENSURE THAT ALL QUESTIONS ARE ANSWERED.".'</div>';
    return $message;
}

add_filter( 'gform_confirmation', 'custom_confirmation', 10, 4 );
function custom_confirmation( $confirmation, $form, $entry, $ajax ) {
    $percent = rgars($entry, 'gquiz_percent');
    $score = rgars($entry, 'gquiz_score');
    if($form['id'] >= '13' && $form['id'] <= '18'){
        if($percent>=80) {
            $_SESSION['course_step'] = $_SESSION['course_step'] + 1;
            $user  = get_user_by('email', $_SESSION['user_course_mail']);
            update_user_meta($user->id, "course_step", $_SESSION['course_step']);
            $_SESSION['course_percent'] = $percent;
            $_SESSION['course_score'] = $score;
            update_user_meta($user->id, "course_percent", $_SESSION['course_percent']);
            update_user_meta($user->id, "course_score", $_SESSION['course_score']);
            $confirmation = "";
            reg_login_redirect();
        }
        else{
            $confirmation = "YOU DID NOT SUCCESSFULLY COMPLETE THE QUIZ. YOUR SCORE IS: ".$score." / 10 (".$percent.".00%). A SCORE OF 80% IS NEEDED. YOU HAD THE FOLLOWING QUESTIONS WRONG:";
 ...
 */
function atelier_child_scripts()
{

    wp_enqueue_script('quiz', get_stylesheet_directory_uri() . '/js/quiz.js', array('jquery'), '20151215', true);
    wp_enqueue_script('bs-min-js', get_stylesheet_directory_uri() . '/js/bootstrap.min.js', array('jquery'), NULL, true);
    wp_enqueue_script('ss-js', get_stylesheet_directory_uri() . '/js/ss.js', array('jquery'), NULL, true);
}

function wbgm_js_variables(){
    $variables = array (
        'ajax_url' => admin_url('admin-ajax.php'),
        'is_mobile' => wp_is_mobile()
        // Usually in here different variable
    );
    echo('<script type="text/javascript">window.wp_data = ' . json_encode($variables) . ';</script>');
}

add_action('wp_head','wbgm_js_variables');


function atelier_child_style()
{
    wp_enqueue_style('bs-min-css', get_stylesheet_directory_uri() . '/css/bootstrap.min.css');
}

add_action( 'wp_enqueue_scripts', 'atelier_child_scripts' );
add_action( 'wp_enqueue_scripts', 'atelier_child_style' );

add_action( 'wp_ajax_quizStep', 'quizStep_callback' );
add_action( 'wp_ajax_nopriv_quizStep', 'quizStep_callback' );

function quizStep_callback() {
...
add_filter('woogoogad_restrict_billing_country_filter',function(){
	
	// AU stands for Australia. This must be a two capital letter code.
	return 'DE';

});

/*Metaboxes for Badge Manager*/

function remove_content_meta() {

   remove_filter( 'rwmb_meta_boxes', 'sf_register_meta_boxes' );

}
add_action( 'after_setup_theme', 'remove_content_meta' );

function rsf_register_meta_boxes() {
    ...
    $meta_boxes[] = array(
        'title'      => __( 'WooCommerce Product Badges', 'textdomain' ),
        'post_types' => 'product',
        'fields'     => array(
            array(
                'name'        => esc_html__( 'Select Badges', 'your-prefix' ),
                'id'          => "selected_badges",
                'type'        => 'post',
                // Post type
    ...
    $meta_boxes[] = array(
        'title'      => __( 'WooCommerce Product Badges', 'textdomain' ),
        'post_types' => 'woo_product_bages',
        'fields'     => array(
            array(
                'name'        => esc_html__( 'Select Category To Assign With This Badge', 'your-prefix' ),
                'id'          => "woo_pro_badges_cat",
            ...
            array(
                'name'        => esc_html__( 'Select Author', 'your-prefix' ),
                'id'          => "woo_pro_published_author",
                'type'        => 'select_advanced',
                'multiple'    => true,
                'placeholder' => esc_html__( 'Select an Author', 'your-prefix' ),
                'options'  => getAllAuthors(),
                'class'      => 'woo_pro_meta_box',
            ),
            array(
                'name'  => __( 'Badge Custom Description Page Link', 'textdomain' ),
                'id'    => 'custom_badge_link',
                'type'  => 'text',
                'class'      => 'woo_pro_meta_box',
            ),
        ),
...
add_filter( 'rwmb_meta_boxes', 'rsf_register_meta_boxes' );

function rsf_build_meta_box() {
    echo'<div class="sf-meta-tabs-wrap"><div id="sf-tabbed-meta-boxes"></div></div>';
}

function rsf_register_meta_box_holder() {
    add_meta_box( 'sf_meta_box', __( 'Meta Options', 'swiftframework' ), 'rsf_build_meta_box', '', 'normal', 'high' );
}
add_action( 'add_meta_boxes', 'rsf_register_meta_box_holder' );
	...
	if (!function_exists('sf_header_aux')) {
        function sf_header_aux($aux) {
            ...
            if ($aux == "left") {
                ...
                $aux_output .= apply_filters('wbgm_logo');
...
if (!function_exists('sf_top_bar')) {
    function sf_top_bar( $sticky = false ) {
        ...
        if ($tb_left_config == "social") {
            ...
            $wbgm_top_info = apply_filters('wbgm_top_info', $tb_left_text);
            if($wbgm_top_info == '') {
                $tb_left_output .= '<div class="tb-text">'.do_shortcode($tb_left_text).'</div>'. "\n";
            } else {
                $tb_left_output .= '<div class="tb-text">'.$wbgm_top_info.'</div>'. "\n";
            }
        }
...
if (!function_exists('sf_enqueue_scripts')) {
    function sf_enqueue_scripts() {
        ...
        wp_register_script('sf-functions-min', SF_LOCAL_PATH_CHILD . '/js/functions.min.js?'.time(), 'jquery', NULL, TRUE);
...
/**-------------------------------*\
 * Bacchus Gold Members List
\---------------------------------*/
add_action( 'init', 'create_post_gold_member_list');
function create_post_gold_member_list() {
    $labels = array(
        'name' => __( 'Gold Member List' ),
        'singular_name' => __( 'Gold Member' )
    );
    ...
    register_post_type( 'wbgm_gmember_list',$args);

}

add_action('post_submitbox_minor_actions','private_gmember');

function private_gmember($post){
    if($post->post_type == 'wbgm_gmember_list'){

...
class VP_Option
{
	...
	public function __construct(array $configs)
	{
		...
		$configs = array_merge(array(
			...
			'layout'                => 'fixed',
			'page_title'            => __( 'Vafpress Options', 'woo_product_badge_manager_txtd' ),
			'menu_label'            => __( 'Vafpress Options', 'woo_product_badge_manager_txtd' ),
			'priority'              => 10,
		), $configs);
		...
		else throw new Exception(__( 'Option Key is required', 'woo_product_badge_manager_txtd' ), 1);
		if(isset($template)) $this->set_template($template);
		else throw new Exception(__( 'Template Array/File is required', 'woo_product_badge_manager_txtd' ), 1);
		if(isset($page_slug)) $this->set_page_slug($page_slug);
		else throw new Exception(__( 'Page Slug is required', 'woo_product_badge_manager_txtd' ), 1);
	...
	{
		VP_WP_Util::admin_notice(__("Development Mode is Active, options' values won't be saved into database.", 'woo_product_badge_manager_txtd'), false);
	}
	...
	function ajax_import_option()
	{
		...
		if($result['status'])
		{
			...
			{
				$result['status']  = false;
				$result['message'] = __("Can not be empty", 'woo_product_badge_manager_txtd');
			}
			else
			{
				...
				{
					$result['status']  = false;
					$result['message'] = __("Invalid data", 'woo_product_badge_manager_txtd');
				}
			}
		}
	...
	function ajax_export_option()
	{
		...
		if($result['status'])
		{
			...
			$result = array(
				'status' => true,
				'message'=> __("Successful", 'woo_product_badge_manager_txtd'),
				'option' => $sr_options,
			);
		}
	...
	function vp_verify_nonce()
	{
		...
		{
			$result['status']  = true;
			$result['message'] = __("Successful", 'woo_product_badge_manager_txtd');	
		}
		else
		{
			$result['status']  = false;
			$result['message'] = __("Unverified Access", 'woo_product_badge_manager_txtd');
		}
		return $result;
	}
	...
	public function init_options_set()
	{
		...
		if( is_string($this->get_template()) and is_file($this->get_template()) )
			...
			throw new Exception(__( 'Invalid template supplied', 'woo_product_badge_manager_txtd' ), 1);
		...
		if( $this->use_util_menu() )
		{
			...
			$util_menu->set_title(__('Utility', 'woo_product_badge_manager_txtd'));
			...
			$restore_section->set_title(__('Restore Default', 'woo_product_badge_manager_txtd'));
			...
			$exim_section->set_title(__('Export/Import', 'woo_product_badge_manager_txtd'));

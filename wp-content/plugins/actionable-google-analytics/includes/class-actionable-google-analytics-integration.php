...
/**
 * Actionable Google Analytics for Woo-commerce stores
 *
...
class WC_Actionable_Google_Analytics extends WC_Integration {
    ...
    public function __construct() {
        ...
        //Thank you page code
        add_action("woocommerce_thankyou", array($this, "thankyou_page_code"));
    ...
    function thankyou_page_code($order_id) {
        ...
        if ($order->get_items()) {
            ...
            $orderpage_trans_Array = array(
                ...
                "tvc_cd" => esc_js($order->get_cart_discount()), //cart discount
    ...
    public function product_detail_view() {
        ...
        if ($product->is_type('variable') && $product->has_child()) {
            ...
            for ($i = 0; $i < sizeof($t_var_ids); $i++) {
                $t_var_metadata = $product->get_child($t_var_ids[$i]);
                $t_var_sku = $t_var_metadata->get_sku(); //get sku
                $t_var_prc = $t_var_metadata->get_price(); //get price
                $t_var_sprc = $t_var_metadata->sale_price; //get sale price
                $t_var_rprc = $t_var_metadata->regular_price; //get regular price
                //get var product weight
                $t_vwt = '';
                if ($t_var_metadata->variation_has_weight) {
                ...
                $prod_var_array[$t_var_ids[$i]] = array(
                    ...
                    "tvc_vtst" => $t_var_metadata->get_total_stock() //total stock with variation count
                );
            }
        } else {
            $prod_var_array = array();
    ...
    public function remove_cart_tracking() {
        ...
        foreach ($woocommerce->cart->cart_contents as $key => $item) {
            ...
            if ($prod_meta->is_type('variable') && $prod_meta->has_child()) {
                ...
                $t_var_metadata = $prod_meta->get_child($item["variation_id"]);
    ...
    public function get_ordered_items() {
        ...
        foreach ($woocommerce->cart->cart_contents as $item) {
            ...
            if ($p->is_type('variable') && $p->has_child()) {
                $t_var_metadata = $p->get_child($item["variation_id"]);
                $t_var_sku = $t_var_metadata->get_sku();
                $t_var_prc = $t_var_metadata->get_price(); //get price
                $t_var_sprc = $t_var_metadata->sale_price; //get sale price
                $t_var_rprc = $t_var_metadata->regular_price; //get regular price

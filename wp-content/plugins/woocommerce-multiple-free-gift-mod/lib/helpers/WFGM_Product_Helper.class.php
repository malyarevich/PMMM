...
class WFGM_Product_Helper
{
	...
	public static function get_product_details( $product_id )
	{
		...
		$wfgm_product_details = array();
		if( ! empty($product_details) && ! empty($product_details->posts) ) {
			$wfgm_product_details['detail'] = $product_details->post;
			$product_image = wp_get_attachment_image_src( get_post_thumbnail_id( $product_details->post->ID ), 'thumbnail' );
			$wfgm_product_details['image'] = isset($product_image[0]) ? $product_image[0] : false;
		}

		return (object) $wfgm_product_details;
	}
	...
	public static function create_gift_variation( $product_id )
	{
		...
		update_post_meta( $post_id, '_wfgm_gift_product', 1 );
	...
	public static function add_free_product_to_cart( $parent_product_id, $product_id, $count = 1 )
	{
		...
		if( count( WC()->cart->get_cart() ) > 0 ) {
            ...
            $so_type_text = WFGM_Settings_Helper::get('type_text', false, 'global_options');
            if( false === $so_type_text ) {
                $so_type_text = WFGM_Common_Helper::translate( 'Type' );
            }

            $so_free_item_text = WFGM_Settings_Helper::get('free_item_text', false, 'global_options');
            if( false === $so_free_item_text ) {
                $so_free_item_text = WFGM_Common_Helper::translate( 'Free Item' );
            }
    ...
    /**
     * Array of id_product and product_Count excluding gift items
     *
     * @since  0.0.0
     * @access public
     * @static
     ...
     * @return array
     */
    public static function get_array_product_and_count()
    {
        $count = 0;
        foreach( WC()->cart->cart_contents as $key => $content ) {
            $is_gift_product = ! empty( $content['variation_id'] );

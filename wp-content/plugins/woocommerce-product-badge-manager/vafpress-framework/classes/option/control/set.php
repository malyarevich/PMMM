...
class VP_Option_Control_Set
{
	...
	public function save($option_key)
	{
		...
		if(update_option($option_key, $opt))
		{
			...
			$result['message'] = __('Saving successful', 'woo_product_badge_manager_txtd');
		...
		else
		{
			...
			if($changed)
			{
				...
				$result['message'] = __('Saving failed', 'woo_product_badge_manager_txtd');
			}
			else
			{
				$result['status']  = true;
				$result['code']    = self::SAVE_NOCHANGES;
				$result['message'] = __('No changes made', 'woo_product_badge_manager_txtd');
			}
		}

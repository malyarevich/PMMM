...
class Guaven_woo_search_analytics
{
  ...
  PRIMARY KEY (`ID`)
    ...
    $this->save_settings();
        ...
        $tabledata = $this->make_table_data();
        $chartdata = $this->make_chart_data();
        require_once plugin_dir_path(dirname(__FILE__)).'admin/view-analytics.php';

        return;
    }

    public function make_table_data()
    {
        global $wpdb;
        ...
        if ($flt == 'popular') {
            ...
            '.$date_and_state[0].' '.$date_and_state[1].' '.$date_and_state[2].' group by keyword order by date_or_count desc limit 100';
        } elseif ($flt == 'popular_uniq') {
            $sql = 'select *,count(keyword) as date_or_count from (select DISTINCT keyword,user_info,state,device_type,ID
  from '.$wpdb->prefix.'woos_search_analytics) a where 1=1 '.$date_and_state[0].' '.$date_and_state[1].' '.$date_and_state[2].' group by keyword order by date_or_count desc';
        } else {
            $sql = 'select user_info,device_type,keyword,state,created_date as date_or_count, ID
  from '.$wpdb->prefix.'woos_search_analytics where 1=1 '.$date_and_state[0].' '.$date_and_state[1].' '.$date_and_state[2].' order by ID desc';
        }
        return $wpdb->get_results($sql);
    }

    public function make_chart_data()
    {
        global $wpdb;
        $date_and_state = $this->build_where();
        $sql = 'select state,count(state) say,created_date,device_type
  from '.$wpdb->prefix.'woos_search_analytics where 1=1  '.$date_and_state[0].' '.$date_and_state[1].'  '.$date_and_state[2].' group by state,created_date order by created_date asc';
        ...
        return array($crkeys, $crdef, $devicetype_labels, $devicetype_values,$date_and_state[4]);
    }
    ...
    public function build_where()
    {
        ...
        if (isset($_POST['state']) and in_array($_POST['state'], array('success', 'fail', 'corrected'))) {
        ...
        return array($date_sql, $state_sql,$device_sql,$flt,$date_interval);
    }
    ...
    {
        add_submenu_page('woocommerce', 'Guaven Woo Search Analytics', 'Search Analytics', 'manage_options', __FILE__, array(
        $this,
        'run',
    ));
    }

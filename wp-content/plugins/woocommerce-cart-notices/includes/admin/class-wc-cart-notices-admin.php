...
class WC_Cart_Notices_Admin {
	...
	private function validate_options() {
		...
		if ( $_POST['minimum_order_amount'] && ( ! is_numeric( $_POST['minimum_order_amount'] ) || (float) $_POST['minimum_order_amount'] < 0 ) ) {
			$this->message_handler->add_error( __( 'Minimum order amount must be positive number, or empty', 'woocommerce-cart-notices' ) );
		}
	...
	private function handle_create_update_cart_notice( $action ) {
		...
		switch ( $notice_type ) {
			...
			case 'products':
				...
				$fields['data']['product_ids']        = explode( ',', $this->get_request( 'product_ids' ) );
			...
			case 'categories':
				$fields['data']['category_ids'] = explode( ',', $this->get_request( 'category_ids' ) );
			break;
	 ...
	 *
	 * @since 1.0.7
	 * @return object notice settings object with data loaded from the current request
	 */

...
<form action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
    ...
    <div class="row">
        ...
        <div class="col-sm-9 cart-items-wrap">
            ...
            <table class="shop_table shop_table cart" cellspacing="0">
                <thead>
                <tr>
                ...
                foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
                    ...
                    if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
                        ...
                        <tr class="<?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
                            ...
                            <td class="product-name" data-title="<?php _e( 'Product', 'swiftframework' ); ?>">
                                <?php
                                if ( ! $_product->is_visible() )
                            ...
                            <td class="product-price" data-title="<?php _e( 'Price', 'swiftframework' ); ?>">
                                <?php
                                echo apply_filters( 'woocommerce_cart_item_price', WC()->cart->get_product_price( $_product ), $cart_item, $cart_item_key );
                                ?>
                            </td>

                            <td class="product-quantity" data-title="<?php _e( 'Quantity', 'swiftframework' ); ?>">
                                <?php
                                if ( $_product->is_sold_individually() ) {
                                    $product_quantity = sprintf( $cart_item['quantity'] . '<input type="hidden" name="cart[%s][qty]" value="%s" />', $cart_item_key, $cart_item['quantity'] );
                                } else {
                            ...
                            <td class="product-subtotal" data-title="<?php _e( 'Total', 'swiftframework' ); ?>">
                                <?php
                                echo apply_filters( 'woocommerce_cart_item_subtotal', WC()->cart->get_product_subtotal( $_product, $cart_item['quantity'] ), $cart_item, $cart_item_key );
                                ?>
                            </td>

                            <td class="product-remove" data-title="<?php _e( 'Remove', 'swiftframework' ); ?>">
                                <?php

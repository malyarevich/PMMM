...
$dev_red_info = $analyticsEngineModel->get_redirects_by_device(
        BISAnalyticsUtil::get_current_month_first_day(), BISAnalyticsUtil::get_current_month_last_day());

$devices_redirects = $dev_red_info['data'];

$json_data = json_encode($devices_redirects);
?>
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-lg-4">
            <div class="jumbotron">
                <h4><?php _e("Redirect by Device Chart", BIS_RULES_ENGINE_TEXT_DOMAIN); ?></h4>
                <span>
                    <?php _e("This chart provides the redirects made from the site to each device. Mouseover on the chart get the percentage of redirects from selected device.", BIS_RULES_ENGINE_TEXT_DOMAIN); ?></span>
            </div>          
        </div>
        <div class="col-lg-8">
            <?php 
            $no_data = "display:block";
            if (!empty($devices_redirects)) { 
                $no_data = "display:none";
            ?>
            <div id="donut_chart_redirect"></div>
            <?php } ?>
...
jQuery(document).ready(function () {
        ...
        jsonData.forEach(function (e) {
            rdeviceArray[count] = [e.device, e.count]; 
            rdevices[count] = e.device;
            count++;
        });
   
        var deviceRedirect = c3.generate({
            bindto: '#donut_chart_redirect',
            data: {
            ...
            donut: {
                ...
                title: "<?php _e("Redirect by Device", BIS_RULES_ENGINE_TEXT_DOMAIN); ?>",
                width: 150
            },
            pie: {
                label: {
        ...
        var options = {
            ...
            data: {
                ...
                bis_report_id: 1,
                bis_report_type_id: 'bis_device_redirects',
        ...
        function showResponse(responseText, statusText, xhr, $form) {
            if (responseText["status"] === "success") {
                jQuery("#donut_chart_redirect").show();
                ...
                jQuery("#donut_chart_redirect").hide();

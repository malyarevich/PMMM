...
<?php if (get_option( 'guaven_woos_data_tracking')!=1) {
  ?>
  ...
  <div>
    ...
    <form action="" method="post">
      <label>Last <input type="number" name="days" style="width:50px" value="<?php echo $chartdata[4];?>"/> days</label>,
  ...
  if (!isset($sufaco[$tdata->state])) $sufaco[$tdata->state]=0;$sufaco[$tdata->state]++;
  }
     ?>
  </table>
  </div>
...
<?php
$i=0;
if (count($chartdata[0])>20) {foreach($chartdata[0] as $key=>$value) {$i++;
  if ($i!=1 and $i!=count($chartdata[0]) and $i!=round(0.5*count($chartdata[0])))
  $chartdata[0][$key]="''";
} }
?>
  new Chartist.Line('.ct-chart', {
    labels: [<?php echo implode(",",$chartdata[0]);?>],
    series: [<?php foreach($chartdata[1] as $key=>$value) echo '['.implode(",",$value).'],
    ';?>]
  }, {
    fullWidth: true,
    chartPadding: {
  ...
  var data = {
    //labels: [<?php echo $chartdata[2];?>],
    series: [<?php echo $chartdata[3];?>]
  };
  ...
  new Chartist.Bar('.ct-bar', {
    ...
    series: [<?php echo "'".(!empty($sufaco['success'])?$sufaco['success']:0)."','".(!empty($sufaco['corrected'])?$sufaco['corrected']:0)."','".
    (!empty($sufaco['fail'])?$sufaco['fail']:0)."'" ;?>]
  }, {
